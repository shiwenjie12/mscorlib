...
namespace System.IO
{
    ...
    public class StreamReader : TextReader
    {
        ...
        // 使用一个1字节缓存和一个4k文件流缓存对于性能是一个不错的处理
        // 当一个超过40MB的文本文件时，即使在一个40 MB的文本文件，
        // 任何性能损失取胜的否定采用4K的缓冲区的分配一个较小的字节数组，从而节省施工时间。
        // 这会破坏自适应缓存，但这是稍快的。
        /// <summary>
        /// 默认的缓存区大小为1024B(1KB)
        /// </summary>
        ...
        //我们不保证线程安全StreamReader，但我们至少应该防止用户想读什么，在同一个线程中异步读是进步。
        [NonSerialized]
        ...
        {           
            ...
            if (t != null && !t.IsCompleted)// 判断当前的异步任务读取是否为空或者是否完成，如果不为空或未完成则抛出异常
        ...
        // StreamReader默认忽略有问题的UTF8的字符，我们不想在在这抛出异常，因为我们想不通过
        // 控制读取错误格式的数据，当我们读的时候，高层次的目标是要宽容编码的错误，当我们写的时候，要非常严格
        // 因此，默认的StreamWriter编码将在错误的时候抛出异常。
        ...
        // if the instance operated on is remote. This is optimised for fields on 搕his?but if a method is Async
