...
namespace System {
    ...
    public abstract class Array : ICloneable, IList, IStructuralComparable, IStructuralEquatable 
    {
        ...
        {
            if (newSize < 0)
                throw new ArgumentOutOfRangeException("newSize", Environment.GetResourceString("ArgumentOutOfRange_NeedNonNum"));
        ...
        /// </summary>
        /// <param name="elementType">要创建的 Array 的 Type。</param>
        /// <param name="length">要创建的 Array 的大小。</param>
        /// <returns></returns>
        ...
        public unsafe static Array CreateInstance(Type elementType, int length)
        {
            ...
            if (length < 0)
                throw new ArgumentOutOfRangeException("length", Environment.GetResourceString("ArgumentOutRange_NeedNonNegNum"));
        ...
        /// </summary>
        /// <param name="elementType">要创建的 Array 的 Type。</param>
        /// <param name="length1">要创建的 Array 的第一维的大小。</param>
        /// <param name="length2">要创建的 Array 的第二维的大小。</param>
        /// <returns></returns>
        ...
        /// </summary>
        /// <param name="elementType">要创建的 Array 的 Type。</param>
        /// <param name="lengths">一个 32 位整数数组，它表示要创建的 Array 中每个维的大小。</param>
        /// <returns>使用从零开始的索引、具有指定 Type 的新的多维 Array，其每个维度都为指定的长度。</returns>
        [System.Security.SecuritySafeCritical]  // auto-generated
        ...
        // is larger than the given search value.
        /// <summary>
        /// 在一维排序数组的值的范围中搜索，使用指定的IComparer接口。
        /// </summary>
        /// <param name="array">排序的一维阵列搜索</param>
        /// <param name="index">起始索引搜索的范围</param>
        /// <param name="length">搜索范围的长度</param>
        /// <param name="value">搜索目标</param>
        /// <param name="comparer">这个IComparer实施时要使用的元素比较。</param>
        /// <returns></returns>
        [Pure]
        [System.Security.SecuritySafeCritical]  // auto-generated
        [ReliabilityContract(Consistency.WillNotCorruptState, Cer.MayFail)]
        ...
        /// <param name="match">Predicate<T>，定义要搜索的元素的条件。</param>
        /// <returns></returns>
        ...
        /// <summary>
        /// 搜索匹配由指定谓词定义的条件的元素，并返回整个数组中的第一个发生。
        /// </summary>
        /// <typeparam name="T"></typeparam>
        ...
        public static int FindIndex<T>(T[] array, int startIndex, int count, Predicate<T> match)
        {
            ...
            if (startIndex < 0 || startIndex > array.Length)
                throw new ArgumentOutOfRangeException("startIndex", Environment.GetResourceString("ArgumentOutOfRange_Index"));
            if (count < 0 || startIndex > array.Length - count)
                throw new ArgumentOutOfRangeException("count", Environment.GetResourceString("ArgumnetOutOfRange_Count"));
            if (match == null)
                throw new ArgumentNullException("match");
        ...
        /// <param name="action">要对 array 的每个元素执行的 Action<T>。</param>
        ...
        public static int IndexOf(Array array, Object value, int startIndex, int count) {
            ...
            if (count < 0 || count > array.Length - startIndex + lb)
                throw new ArgumentOutOfRangeException("count", Environment.GetResourceString("ArgumentOutRange_Count"));

            int retVal;
            bool r = TrySZBinarySearch(array, startIndex, count, value, out retVal);
            if (r)
                return retVal;
        ...
        public static void Sort(Array keys,Array items,int index,int length,IComparer comparer)
        {
            if (keys == null)
                throw new ArgumentNullException("keys");
            if (keys.Rank != 1 || (items != null && items.Rank != 1))
                throw new RankException(Environment.GetResourceString("Rank_MultiDimNotsSupported"));
            if (items != null && keys.GetLowerBound(0) != items.GetLowerBound(0))
                throw new ArgumentException(Environment.GetResourceString("Arg_LowerBoundsMustMatch"));
            if (index < keys.GetLowerBound(0) || length < 0)
                throw new ArgumentOutOfRangeException((length < 0 ? "length" : "index"), Environment.GetResourceString("ArgumentOutOfRange_NeedNonNum"));
            if (keys.Length - (index - keys.GetLowerBound(0)) < length || (items != null && (index - items.GetLowerBound(0)) > items.Length))
                throw new ArgumentException(Environment.GetResourceString("Argument_InvalidOffLen"));
