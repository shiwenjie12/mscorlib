...
namespace System.Collections {
    ...
    public class Queue : ICollection, ICloneable {
        ...
        {
            if (capacity < 0)
                throw new ArgumentOutOfRangeException("capacity", Environment.GetResourceString("ArgumentOutOfRange_NeedNonNum"));
            if (!(growFactor >= 1.0 && growFactor <= 10.0))
                throw new ArgumentOutOfRangeException("growFactor", Environment.GetResourceString("ArgumentOutOfRange_QueueGrowFactor", 1, 10));
            Contract.EndContractBlock();
            _array = new Object[capacity];
        ...
        public virtual void CopyTo(Array array,int index)
        {
            ...
            numToCopy -=firstPart;
            if(numToCopy > 0)
                Array.Copy(_array,0,array,index+array.Length - _head,numToCopy);
        }
