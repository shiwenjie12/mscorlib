...
#pragma warning restore 618        
    ...
    internal static class HashHelpers
    {
        ...
        //质数表作为哈希表大小。
        //典型的调整算法会选择最小的质数这个数组容量比以前的两倍大。
        //假设我们的散列表目前产能x和足够的元素添加这样一个调整需要发生。
        //首先调整计算2 x然后发现表中第一个'大于2 x,即如果启动命令p_1、p_2,……,p_i,…,它发现p_n p_n-1 < 2 x < p_n。
        //翻倍是重要的保护渐近散列表的操作,比如添加复杂性。有一个'保证双散列不会导致无限循环。
        //即你的哈希函数将h1(键)+ i * h2(关键),0 < =我<大小。h2和大小必须互质。
