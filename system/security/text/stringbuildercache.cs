...
{
    /// <summary>
    /// 提供一个重复使用缓冲实例stringbuilder
    /// 他是性能客观的，减少实例的构造和收集数目
    /// </summary>
    internal static class StringBuilderCache
    {
        ...
        public static StringBuilder Acquire(int capacity = StringBuilder.DefaultCapacity)
        {
            if(capacity <= MAX_BUILDER_SIZE)//判断是否超过最大大小
            {
                ...
                if (sb != null)
                {
                    ...
                    // 避免stringbulder块被获取一个新的StringBuilder而分裂当请求的大小超过当前容量
                    if(capacity <= sb.Capacity)
                    {
                        // 将当前StringBuider设置为空，并清空内容
        ...
        {
            if (sb.Capacity <= MAX_BUILDER_SIZE)//如果sb.Capacity <= MAX_BUILDER_SIZE 表示 sb是由 实例缓存 转换而来的
            {
