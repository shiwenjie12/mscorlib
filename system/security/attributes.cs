...
#pragma warning restore 618
    ...
    // At type-scope, all rule checks will be suppressed for members within the type and for calls made against the type.
    // At member level (e.g. field and method) the code will be treated as public - i.e. no rule checks for the members.
    // 表名代码可能包含违反安全关键的规则（例如，从关键的非公开透明，透明的非公开批评，等等）。被审计的安全问题和被认为是安全清洁。
    // 也表明,被认为是SecurityCritical的代码。这个属性的作用是如果代码标记[SecurityCritical][SecurityTreatAsSafe]。
    // assembly-scope,所有规则检查将被抑制在装配和装配要求。
    // 在type-scope,所有规则检查将抑制成员在类型和要求类型。
    // 在会员级别(如字段和方法)的代码将被视为公共成员——即没有规则检查。
