...
namespace System.Security
{
    ...
    //  此类是用于安全系统中简单 XML 对象模型的轻量实现，不作为常规 XML 对象模型使用。
    //  注意：由于性能方面的原因，只有在以 XML 文本格式对元素编码时才进行字符有效性检查，而不是对每个属性或方法集都进行检查。静态方法允许在所需位置进行显式检查。
    //
    /// <summary>
    /// 表示编码安全对象的 XML 对象模型。此类不能被继承。
    /// </summary>
    [Serializable]
    ...
    sealed public class SecurityElement : ISecurityElementFactory
    {
        ...
        /// 标志的属性
        /// </summary>
        private const int c_AttributesTypical = 4 * 2;  // 4个属性，每个属性的2个字符串
...
//////////////
        ...
        public Hashtable Attributes
        {
            get
            {
                if (m_lAttributes == null || m_lAttributes.Count == 0)//如果属性为空，或属性的长度为0.则返回为空
                {
                    return null;
                }
                else
                {
                    Hashtable hashtable = new Hashtable(m_lAttributes.Count / 2);//初始化长度为属性长度/2的Hashtable
        ...
        {
            get//获取XML 元素子元素子元素SecurityElement
            {
            ...
            set//设置XML 元素子元素
            {
                if (value != null)//如果值为空，初始化XML元素子元素
                {
        ...
        internal ArrayList InternalChildren
        {
            get
            {
                // 小心!这个数组列表可以包含SecurityElements和其他ISecurityElementFactories。如果你想获得一个一致的SecurityElement看来,调运get_Children。
                return m_lChildren;
            }
        }  
